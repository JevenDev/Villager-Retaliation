...
public final class VillagerChatEffectRenderer {
    ...
    public static void render(GuiGraphics graphics, Minecraft minecraft) {
        ...
        for (int index = 0; index + chat.chatScrollbarPos < totalLines && index < linesPerPage; index++) {
            ...
            int textAlpha = (int) (255.0D * timeFactor * opacity);
            int backgroundAlpha = (int) (255.0D * timeFactor * backgroundOpacity);
            visibleLineCount++;
            if (textAlpha <= 3) {
                continue;
            }
        ...
        if (chat.isChatFocused()) {
            renderScrollbar(graphics, chat, chatWidth, chatBottom, totalLines, lineHeight, visibleLineCount);
        }
