...
public final class VillagerReputationNetworking {
    ...
    public static void registerPayloads(RegisterPayloadHandlersEvent event) {
        ...
        network.safePlayToClientThreaded(
                DuelFxStatePayload.TYPE,
                DuelFxStatePayload.STREAM_CODEC,
                "com.jvn.villagerretaliation.client.duel.DuelFxClient",
                "accept"
        );
        network.safePlayToClientThreaded(
        ...
        network.playToServer(
                HiredAnimalHandlingOptionPayload.TYPE,
                HiredAnimalHandlingOptionPayload.STREAM_CODEC,
                (payload, context) -> ToucanNetwork.enqueue(context, () ->
                        ToucanNetwork.withServerPlayer(context, player -> VillagerInteractionService.handleAnimalHandlingOptionRequest(
                            player,
                            payload.entityId(),
                            payload.optionId()
                    )))
        ...
        network.playToServer(
                ItemFilterCombinationChangePayload.TYPE,
                ItemFilterCombinationChangePayload.STREAM_CODEC,
                (payload, context) -> ToucanNetwork.enqueue(context, () ->
                        ToucanNetwork.withServerPlayer(context, player ->
                                com.jvn.villagerretaliation.item.VillagerItemFilterItem.handleCombinationChange(
                                        player,
        ...
        network.playToServer(
                FilterPolicyChangePayload.TYPE,
                FilterPolicyChangePayload.STREAM_CODEC,
                (payload, context) -> ToucanNetwork.enqueue(context, () ->
                        ToucanNetwork.withServerPlayer(context, player -> {
                            var field = payload.requestedField();
                            if (field != null
                                    && player.containerMenu instanceof com.jvn.villagerretaliation.inventory.VillagerFilterPolicyMenu menu) {
                                menu.applyPolicyChange(field, payload.value());
                            }
                        }))
        );
        network.playToServer(
                        ...
                        ToucanNetwork.withServerPlayer(context, player -> {
                            if (player.containerMenu instanceof com.jvn.villagerretaliation.inventory.VillagerItemFilterMenu menu) {
                                menu.setGhostEntry(payload.slot(), payload.entry());
                                    ...
                                    && player.containerMenu instanceof com.jvn.villagerretaliation.inventory.VillagerAttributeFilterMenu menu) {
                                menu.setReference(payload.entry());
                            }
                        }))
        );
        network.playToServer(
        ...
        network.playToServer(
                VillagerConversationActivityPayload.TYPE,
                VillagerConversationActivityPayload.STREAM_CODEC,
                (payload, context) -> ToucanNetwork.enqueue(context, () ->
                        ToucanNetwork.withServerPlayer(context, player -> VillagerInteractionService.handleConversationActivity(
                                player,
                                payload.entityId()
                        )))
        );
    ...
    public static void sendServerConfig(ServerPlayer player) {
        trySendToPlayer(player, new ServerConfigSyncPayload(
                ...
                VillagerRetaliationConfig.SHOW_QUEST_INDICATORS.get(),
