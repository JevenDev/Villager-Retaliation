window.VR_DEVELOPER_WIKI_DATA = {
  ...
  "pages": [
    {
      ...
      "source": "wiki/dev/content/Pack-Development.md",
      "sourceKind": "wiki",
    ...
    },
    {
      ...
      "source": "wiki/dev/content/JSON-Reference.md",
      "sourceKind": "wiki",
      "group": "Getting Started",
      "icon": "braces",
      ...
      "text": "JSON Reference This page covers the shared authoring rules used across Villager Retaliation JSON. Stable Ids Give entries a stable id whenever you may want to: override that entry later translate it in another locale remove it with a follow up datapack read cleaner debug output Example: text vs lines Use text for one output. Use lines when the same rule should randomly say one of several variations. replace and remove Top level replace: true clears the previously loaded pool for that system before the file is applied. Some systems also support entry level removal by id. Builder structures also support removal by structure id: Arrays and Single Values Many fields accept one value or several values. When in doubt, prefer arrays. They are clearer and easier to extend later. Reputation Filters These fields show up in several systems: Field Meaning reputation levels One or more named tiers such as trusted or hostile min reputation Lowest numeric or named reputation allowed max reputation Highest numeric or named reputation allowed Named tiers commonly used in docs: Example: Item and Tag Selectors Use item ids for exact matches: Use tags with when any item in the tag should count: The same pattern is used in sell prices, gifts, pacification, and some forced dialogue payment selectors. Item stack predicates Item selectors in sell prices, gifts, pacification payments, dialogue item payments, quest item objectives, proof items, and encounter retrieve item objectives can also distinguish stacks by data components, custom NBT like data, and remaining durability: components is keyed by registered data component ID. A normal value is an exact codec backed match; numeric components also accept { \"min\": ..., \"max\": ... }. custom data is a shortcut for the minecraft:custom data component and uses recursive subset matching, so unrelated keys on the actual stack are allowed. nbt is an alias for custom data. durability is remaining durability, not raw damage, and accepts min, max, or both. Player item dialogue conditions use the prefixed fields player item components, player item custom data, and player item nbt; the equivalent held item aliases are accepted. Root quest proof items use proof item components, proof item durability, proof item custom data, and proof item nbt. All other locations above use the unprefixed names. Currency Villager Retaliation's hire payments, payment boxes, wallet deposits, wallet UI, default currency drops, and emerald default skill trade costs use: Built in default: Fields: Field Meaning item Primary currency item. Refunds, wallet deposits, drops, and emerald default skill trade costs use this item. accepted items / items Extra item ids accepted as equivalent payment. accepted tags / tags Item tags accepted as equivalent payment. Prefixing with is optional here. name Singular display name used in notices. plural name Plural display name used in notices. wallet label Label shown in the villager interaction wallet line. text color / wallet text color / wallet color / color Hex or named color for the villager interaction wallet number. Defaults to 55ff55. Payment box recipes and client side \"hold currency\" checks also use the villagerretaliation:currency item tag: Keep that tag aligned with your currency item so crafting recipes, payment boxes, and client hints all agree. Conditions conditions are the preferred way to express complex logic in beta.13 content. A condition array usually means all listed conditions must pass. Condition Catalog These canonical condition types are shared by normal dialogue, dialogue trees, forced dialogue, quest availability and active gates, quest triggers, and context aware quest pools where the surrounding system has the required live or saved context. Canonical type Accepted aliases Use all all of, and Every nested condition must match any any of, or At least one nested condition must match not Invert one nested condition reputation Reputation tier or numeric range memory Durable event tags or built in memory kinds family Known family relationship relationship Current or past social relationship recruitment memory Saved recruitment journey details villager age Adult or baby state social attribute attribute, stat Charm, Guts, Kindness, Knowledge, or Proficiency range skill Villager skill value or rank villager level trade level Villager trade level 1 through 5 quest Quest state quest fact quest tag, quest variable, quest counter, quest stage, fact, stage Durable scoped quest fact selected choice choice selected, response selected, quest choice selected Recorded dialogue response transition stage history quest stage history, visited stage Recorded quest stage transition player item item, held item Player inventory, slot, component, durability, or enchantment predicate villager equipment equipment, armed Current villager armed or unarmed state biome Exact biome or biome tag dimension Current dimension advancement advancements Player advancement completion; all defaults to true scoreboard score Player scoreboard objective value or range nearby entity nearby entities, entity nearby Entity type/tag count around villager or player village village presence Tracked village presence or key trigger payload event payload, quest trigger payload Values carried by the current quest trigger mood villager mood Villager mood and intensity weather Clear, rain, or thunder state time time of day Current time band Malformed and unknown conditions fail closed and appear in datapack diagnostics. Conditions that require live player, provider, world, village, or trigger context also fail when that context is unavailable. This matters especially for active quests whose provider may be unloaded. World and inventory conditions can be combined directly: Trigger Payload Conditions trigger payload is meaningful only while a quest trigger is being dispatched. It can match the canonical event, common scalar fields, arbitrary any, all, and not value maps, criterion data, or reputation bounds: Common shorthand keys include mob, entity, block, item, gift reaction, event villager, event villager type, trade cost a, trade cost b, trade result, criterion, and memory tag. A primitive entry under data is matched as criterion . Payload keys and values are normalized to lowercase. Use conditions when the older one off helper flags start to pile up. Mood Conditions Use mood conditions for current villager mood gates. Active quests can evaluate these from saved villager mood state when the issuing villager is unloaded. Legacy equipment flags such as requires villager armed, requires villager unarmed, requires witness armed, and requires witness unarmed are live context gates for dialogue, notification, loot, gift, forced dialogue, and pacify resources. They are not quest conditions and are not evaluated from saved active quest state. Quest Facts Use quest fact conditions for durable story flags, branch choices, and counters written by quest or dialogue actions. Scopes: Scope Meaning player Stored for the current player across the world world Stored once for the whole save quest Stored for the current player and a quest id villager Stored on the current villager id village Stored on the resolved village area, or the current villager position fallback Variables and counters use key plus value, min, or max: Quest stages are shorthand for scope: \"quest\", key: \"stage\", and a stage value: Use all of, any of, and not around quest fact conditions for larger branch logic. Quest offers can use the same condition shape: parent gates a quest behind a completed parent quest for the current player. offer.conditions gates whether the quest can be offered at all. rules.active.conditions controls whether an already active quest can currently progress. Branch locks close unchosen paths: exclusive group makes sibling quests in the same group mutually exclusive. exclusive on accepts started or completed. blocks on start, blocks on completion, and blocks explicitly consume named quests. A locked quest matches quest state branch locked and gets the quest scoped tag villagerretaliation:quest branch locked plus variables blocked by, blocked on, and exclusive group. Quest Module V2 New quests should use schema: \"villagerretaliation:quest/v2\" under data/ /quests/. A simple playable quest can keep provider filters, availability, stages, objectives, dialogue slots, responses, scenes, rewards, events, and tracker UI in one file. Required top level fields: Field Meaning schema Must be villagerretaliation:quest/v2 id Full quest id, such as my pack:bread delivery provider Provider type and filters, usually villagerretaliation:villager entry stage First stage id stages Array of stage objects Common optional fields: Field Meaning metadata title, description, title key, description key, questline, tags, parent, show locked adventure hint availability Selection weight, repeat and cooldown rules, active quest caps, abandonment, locking, completion scope, and active gates lifecycle Actions, transitions, and optional dialogue attached to quest or stage lifecycle events dialogue Root level inline or extracted dialogue slots target Structure target, dimension, search radius, discovery radius, and proof item events Quest level triggers with conditions, payload matching, priority, chance, weight, cooldown, and exclusivity rewards XP, reputation, gossip, loot, memory event, or reward actions ui Tracker text, icon, color, outline color, progress, placeholders, priority, and hidden flag external scenes Resource ids for extracted dialogue tree scenes Set metadata.show locked adventure hint to false when a quest should not appear as a locked preview in the villager Adventures menu before its offer requirements are met. Each stage requires id and objectives. Stages can also define complete when, next, dialogue, responses, scenes, events, entry actions, exit actions, rewards, ui, and metadata. Dialogue slots such as offer, reminder, and turn in can be inline: Or extracted: For responses, use one transition source: direct response fields, a transition object, or a transition action. Do not mix direct next/stage/scene/complete fields with a transition action on the same response. Validate standalone quest modules with: Regenerate the generated authoring schema and registry metadata with: Shared Actions Dialogue trees, quest triggers, and villager event triggers use the same actions shape for most state changes. Common action types: Type Important fields quest quest or quest id, action: start, remind, turn in, abandon, or block notification trigger, text forced dialogue forced dialogue experience amount or experience reputation amount or reputation gossip amount, gossip, or gossip reputation memory memory event, optional memory scope: villager, village, or both loot loot table tracker flash tracker draw weapon Optional duration ticks, duration seconds, or duration days; defaults to 10 seconds set tag tag or set tag, optional scope, optional quest clear tag tag or clear tag, optional scope, optional quest set variable key or variable, value, optional scope, optional quest set stage stage, optional quest. Stores quest scoped branch state counter key or counter, optional amount, by, or delta, optional scope, optional quest Quest facts default to quest scope when the action has a quest id or is inside a quest owned trigger. Otherwise they default to player scope. draw weapon equips the best usable weapon carried by the acting villager for the configured duration. It does not set an attack target, start retaliation, or change reputation. Dialogue tree response actions can use it when a particular player response should make the villager visibly arm themselves: Memory actions default memory scope to both. villager remembers the event only for the acting villager, village writes only to the tracked village footprint containing the event, and both writes each available bucket. Quest reward shorthand using only memory event also defaults to both. Use action: \"block\" when a dialogue choice or trigger should close a path immediately. The target quest becomes branch locked and receives villagerretaliation:quest branch locked. Shared Text Variants And Usage Normal dialogue lines, keyed messages, openings, closings, pacify text, and dialogue tree node or response text can use rich objects in lines or variants: A variant supports id, text or line, text key or line key, priority, chance, weight, conditions, metadata, and usage. Parent metadata and usage values are inherited, then the variant adds or overrides them. Usage fields are cooldown or a duration suffix, max uses, once, anti repeat, and scope. Scopes are player villager, player, villager, village, dimension, and world. Counts and timestamps are durable world save data, so stable IDs are part of save compatibility. Metadata fields are topic, tags, routing tags, anti repeat groups, questline, quest, stage, and notes. General classification tags, routing tags, and anti repeat groups are separate domains; do not use one as an accidental substitute for another. See Dialogue Dialogue.md rich text variants for the complete authoring example. Weights and Priority Candidate based dialogue, rich text, forced dialogue definitions, and quest triggers share this arbitration order: 1. Discard ineligible candidates and candidates with weight: 0. 2. Evaluate higher priority tiers first. 3. Apply each candidate's chance independently. 4. Use weight to choose an order among survivors in that tier. 5. Fall through to lower priorities when the higher tier has no survivor. For normal dialogue lines, specificity weight is an explicit opt in bonus: the effective weight is the authored weight plus the matched line's specificity score multiplied by specificity weight. Its default is 0, so adding more filters does not silently make a line more likely. Example: Use priority for precedence, chance for an independent probability gate, and weight for relative odds among candidates that reached the same tier. Message Keys When several rules should share the same localized text, move the wording into a keyed message and reference it with text key. Canonical Naming Prefer the current documented field names even if compatibility aliases still work. For new content, that usually means: trigger instead of older event aliases world text kind for notifications request on dialogue options and lines conditions for complex logic Troubleshooting Example If a file appears valid but nothing happens, strip it back to a bare minimum: If that works, the problem is in the filters, not the path or loader.",
      "headings": [
        {
          "level": 2,
          "title": "Stable Ids"
        ...
        {
          "level": 3,
          "title": "Trigger Payload Conditions"
        },
        {
          "level": 3,
        ...
        {
          "level": 2,
          "title": "Shared Text Variants And Usage"
        },
        {
          "level": 2,
    ...
    },
    {
      "slug": "first-quest",
      "file": "First-Quest.md",
      "source": "wiki/dev/content/First-Quest.md",
      "sourceKind": "wiki",
      "group": "Getting Started",
      "icon": "route",
    ...
    },
    {
      ...
      "source": "wiki/dev/content/Datapack-Generator.md",
      "sourceKind": "wiki",
      "group": "Getting Started",
      "icon": "hammer",
    ...
    },
    {
      ...
      "source": "wiki/dev/content/Example-Packs.md",
      "sourceKind": "wiki",
      "group": "Getting Started",
      "icon": "folder-down",
      "title": "Example Packs",
    ...
    },
    {
      "slug": "dialogue",
      "file": "Dialogue.md",
      "source": "wiki/dev/content/Dialogue.md",
      "sourceKind": "wiki",
      "group": "Dialogue",
      "icon": "message-square-text",
      "title": "Dialogue",
      "description": "Talk options, rich text variants, durable usage rules, metadata tags, tuning, and conversation text.",
      ...
      "text": "Dialogue Normal dialogue powers the Talk menu, reusable reply pools, keyed text, openings, closings, and pacify lines. Paths Dialogue can live anywhere under: Beta.13 works best with typed folders: Bundle files still work, but folderized files are easier to translate and override. Sections Section Use it for options Talk menu buttons shown to the player lines Villager replies and response pools messages Shared keyed text used by text key or other systems openings First line when a conversation starts closings Final line when a conversation ends pacify Spoken lines used while calming a hostile villager Example: Custom Talk Option Dialogue option IDs and labels are limited to 128 characters so they can be sent safely to clients. Example: Shared Message Text Use messages when several rules should share the same wording. Then point a line at it: Example: Opening Example: Closing Openings and closings can also react when the player displays an ominous banner: worn directly in the head slot, attached to a worn helmet, or applied to a shield in either hand. Use requires ominous banner, and optionally narrow the speaker's durable village allegiance with village allegiance or village allegiances (known, unknown, or unaffiliated): Two item tags make ominous symbol recognition extensible: Tag Purpose villagerretaliation:ominous banner pattern carriers Items whose banner patterns component should be compared with the vanilla ominous design. It contains banners and shields by default. Add compatible modded shields or wearable banner items here. villagerretaliation:ominous banner equivalents Items that always count as displaying the ominous symbol, without requiring banner pattern components. Add custom insignia, uniforms, masks, or other modded gear here. For other gear specific dialogue, openings and closings accept the same player item, player items, player item tag, player item tags, player item slot, and player item slots filters as normal dialogue lines. Slots can be main hand, off hand, hands, armor, hotbar, inventory, accessories, equipment, or any. The accessories slot reads equipped Curios or Accessories items when either optional mod is installed; equipment and any include them as well. Item filtered conversation text can use placeholders such as {player item}, {player item id}, and {player item slot}. These filters can require stack data with player item components, player item custom data, or player item nbt; held item components, held item custom data, and held item nbt are aliases. Component and custom data predicates combine with the existing item, tag, slot, durability, and enchantment checks. See Item stack predicates JSON Reference.md item stack predicates. Example: Pacify Line The items used for pacification live in Pacification Pacification.md. The spoken line lives in dialogue. Example: Profession Specific Line Folder paths can communicate ownership clearly: You can still include explicit professions filters when needed, but the path itself is already a good organizational hint. Rich Text Variants A lines or variants array can contain objects instead of plain strings. This lets each wording have its own stable ID, eligibility conditions, priority, chance, weight, metadata, and durable usage policy: Plain string arrays remain valid. Their generated stable IDs are the entry ID for a single line or line for multiple lines. An explicit variant ID becomes , which is safer when lines are reordered later. Variant selection uses the same contract across normal dialogue and dialogue tree text: 1. Remove variants whose conditions or usage policy do not match. 2. Try higher priority tiers first. 3. Apply each candidate's chance. 4. Select among surviving candidates by weight. 5. If every candidate in a priority tier misses its chance, try the next tier. weight: 0 disables a candidate. chance is clamped from 0.0 to 1.0. Durable Usage Rules Use usage when a line should rest, run only a limited number of times, or avoid immediately repeating: Field Meaning cooldown, cooldown ticks, cooldown seconds, cooldown days Minimum saved time before the same stable text ID can be selected again max uses Lifetime use limit in the selected scope; 0 means unlimited once Shorthand for max uses: 1 anti repeat Prefer a different recent variant when one is available; defaults to true scope player villager (default), player, villager, village, dimension, or world Usage counts and timestamps are stored in world save data. Put usage on the parent entry to provide defaults to all variants, then override it on an individual variant when needed. The legacy top level cooldown , max uses, and once fields on normal dialogue lines remain supported. Dialogue Metadata And Tags metadata is inherited from the file root into entries and then into rich variants. It keeps classification, routing, and anti repeat intent separate: Field Purpose topic Human readable normalized topic; also the fallback anti repeat group tags General classification tags routing tags Tags used specifically to route candidates anti repeat groups Groups whose recent use should discourage repetition questline, quest, stage Optional quest ownership and stage context notes Authoring notes Tags are normalized to lowercase while preserving meaningful separators such as :, /, ., and . When routing tags is omitted, general tags beginning with route., route:, routing., or routing: act as routing tags. Dialogue Frequency Tuning Built in conversation probabilities are datapack values under: Files are additive. When several resources define the same normalized key, the later loaded value wins. Values must be finite numbers; probability consumers clamp them to 0.0 through 1.0. Current built in keys and defaults: Key Default : opening.long absence.minimum days 3 opening.long absence.base chance 0.4 opening.long absence.chance per day 0.1 opening.long absence.max chance 0.85 memory.gift.question chance 0.45 memory.gift.greeting chance 0.35 memory.gift.opening chance 0.3 memory.container theft.question chance 0.4 memory.container theft.greeting chance 0.25 memory.container theft.opening chance 0.25 story hint.vague chance 0.12 story hint.biome name chance 0.24 story hint.precise biome chance 0.34 story hint.structure rumor chance 0.46 story hint.precise structure chance 0.58 cartographer map.royalty chance 0.09 cartographer map.revered chance 0.06 cartographer map.respected chance 0.03 raid.story chance 0.35 reputation.repeat greeting chance 0.15 reputation.question.trusted chance 0.55 reputation.question.neutral chance 0.3 reputation.story.trusted chance 0.65 reputation.story.neutral chance 0.4 reputation.story.profession bonus 0.15 reputation.joke.royalty chance 0.85 reputation.joke.trusted chance 0.7 reputation.joke.neutral chance 0.5 reputation.joke.suspicious chance 0.35 reputation.joke.hostile chance 0.25 reputation.joke.despised chance 0.15 reputation.joke.nitwit bonus 0.1 reputation.joke.missed response chance 0.3333333333 reputation.positive response chance 0.25 Good Defaults Keep one idea per file when possible. Use stable id values. Give rich variants explicit stable IDs before shipping a pack. Prefer conditions once several helper flags are stacking up. For request specific patterns, see Dialogue Requests Dialogue Requests.md. The command /vr admin dialogue explain [option id] reports which request and filters caused a line to match or be rejected.",
      "headings": [
        {
          "level": 2,
          "title": "Paths"
        ...
        {
          "level": 2,
          "title": "Rich Text Variants"
        },
        {
          "level": 2,
          "title": "Durable Usage Rules"
        },
        {
          "level": 2,
          "title": "Dialogue Metadata And Tags"
        },
        {
          "level": 2,
          "title": "Dialogue Frequency Tuning"
        },
        {
          "level": 2,
          "title": "Good Defaults"
    ...
    },
    {
      ...
      "source": "wiki/dev/content/Dialogue-Requests.md",
      "sourceKind": "wiki",
      "group": "Dialogue",
    ...
    },
    {
      ...
      "source": "wiki/dev/content/Dialogue-Trees.md",
      "sourceKind": "wiki",
      "group": "Dialogue",
      "icon": "git-fork",
      ...
      "headings": [
        ...
        {
          "level": 2,
          "title": "Conditions, Metadata, And Rich Text"
        },
        {
          "level": 2,
    ...
    },
    {
      ...
      "source": "wiki/dev/content/Forced-Dialogue.md",
      "sourceKind": "wiki",
      "group": "Dialogue",
      ...
      "headings": [
        ...
        {
          "level": 2,
          "title": "Shared Conditions And Rich Variants"
        },
        {
          "level": 2,
          "title": "Replacing Or Removing Built-Ins"
    ...
    },
    {
      ...
      "source": "wiki/dev/content/Notifications.md",
      "sourceKind": "wiki",
      "group": "Dialogue",
      "icon": "bell",
      "title": "Notifications",
    ...
    },
    {
      ...
      "source": "wiki/dev/content/Notification-Triggers.md",
      "sourceKind": "wiki",
      "group": "Dialogue",
      "icon": "zap",
    ...
    },
    {
      "slug": "event-tags",
      "file": "Event-Tags.md",
      "source": "wiki/dev/content/Event-Tags.md",
      "sourceKind": "wiki",
      "group": "Dialogue",
      "icon": "tags",
      "title": "Event Tags",
    ...
    },
    {
      ...
      "source": "wiki/dev/content/Villager-Event-Triggers.md",
      "sourceKind": "wiki",
      "group": "Dialogue",
      "icon": "radio",
    ...
    },
    {
      ...
      "source": "wiki/dev/content/Localization.md",
      "sourceKind": "wiki",
      "group": "Dialogue",
      "icon": "languages",
      "title": "Localization",
    ...
    },
    {
      "slug": "quests",
      "file": "Quests.md",
      "source": "wiki/dev/content/Quests.md",
      "sourceKind": "wiki",
      "group": "Quests & Scenes",
      "icon": "scroll-text",
      "title": "Quests",
      "description": "Quest module v2 providers, rotating pools, active caps, payload-aware triggers, objectives, branches, rewards, and diagnostics.",
      ...
      "text": "Quests Quest module v2 is the preferred shape for new quest datapacks. A v2 module can define the provider, availability, lifecycle, stages, objectives, dialogue, responses, transitions, events, rewards, and tracker UI in one file. Legacy v1 quest files are still supported. Keep existing v1 packs working, but use v2 for new simple quests and for migrations where you want dialogue and quest state to live together. Each quest run receives a saved unique ID before its first actions run. A solo run belongs to one player. A party run uses one shared ID for the party. Persistent scenes use this saved ID so a reload resumes the same scene instead of starting a duplicate. Paths The folder path is for organization and overrides. It does not create a questline by itself. One File Quest This is a complete playable quest. It needs no external dialogue tree. Validate standalone quest examples with: Main Parts Section Purpose schema Must be villagerretaliation:quest/v2 for v2 modules id Stable quest resource id used by saves, commands, dialogue, and overrides metadata Player facing title, description, questline, tags, and legacy parent convenience provider Who can offer or own the quest availability Ordered prerequisites, repeat limits, abandonment, cooldowns, locking, and active gates target Optional world target such as a structure search entry stage Authoritative first stage id. A later stage named started does not override it stages Objectives, stage local dialogue, responses, scenes, events, and UI events Quest level triggers that run while the quest exists rewards XP, reputation, gossip, loot, memory events, or reward actions ui Tracker text, icon, progress, placeholders, title and outline colors, and priority external scenes Optional external dialogue scene resources used by this module Quest Tag Taxonomy metadata.tags classifies quests for journal filters, quest board pools, authoring tools, diagnostics, and content tests. Tags describe a quest; they do not replace authoritative fields such as availability.repeatable, completion scope, ui.hidden, ui.priority, provider filters, or objective definitions. Journal Presentation Root ui fields are live journal data, not authoring only hints: priority sorts otherwise equivalent journal entries, color tints their titles, outline color opts into a ToucanLib 1px title outline, and hidden suppresses the quest from the journal and HUD. When omitted, title text defaults to unoutlined black. Both authored colors accept RRGGBB or named Minecraft chat colors. The synchronized journal also carries metadata.questline and metadata.tags; press / in the journal to search titles, descriptions, objectives, questlines, and tags. Active quests with availability.expiration.after ticks show a live remaining time countdown. A located structure or location objective publishes its saved dimension and coordinates as a waypoint in both the selected journal entry and tracked HUD, including live distance while the player is in the same dimension. Completed journal history entries show how long ago that run completed. Quest Pools Quest pools turn quest tags or explicit quest IDs into bounded rotating offers. Put pool resources under data/ /quest pools/. A pool only controls quests it claims; quests not claimed by any currently matching pool keep their normal availability. Selection is deterministic for the pool, scope key, and refresh epoch. scope accepts player, village, provider, dimension, or world. refresh and its ticks, seconds, and days forms set the epoch length. anti repeat rotations avoids recent deterministic selections when enough alternatives exist, then backfills so avoidable repeats do not leave the board short. Candidate controls: Field Meaning quests Explicit quest IDs any tags Quests with at least one listed tag all tags Quests with every listed tag exclude quests, exclude tags Remove candidates before selection match any (default) lets any non empty selector claim a quest; all requires every supplied selector family max offers Maximum selected quests, from 1 through 64 enabled Disable the definition without deleting it remove Remove a previously loaded pool with the same id A pool with no positive selector claims no quests. Quest availability, provider filters, active capacity, prerequisites, and other ordinary start rules are evaluated before pool selection. Weighting and diversity: An explicit weights value replaces the calculated weight for that quest; 0 excludes it. Otherwise the weight is default weight multiplied by the quest's availability.weight or selection weight. Every matching weight rules multiplier is applied. A rule can filter by tags and shared conditions. tag quotas caps how many selected quests may carry each listed tag. Selection is weighted without replacement. Pool level conditions decide whether a pool participates for the current player, provider, village, dimension, and world context. When several pools claim the same quest, selection by any applicable non exclusive pool allows it. If an applicable claiming pool is exclusive: true, only exclusive pools at the highest exclusive priority are considered for that quest. The built in villagerretaliation:quest board is village scoped, refreshes every Minecraft day, shows at most three eligible pool.quest board quests, avoids the previous two rotations where possible, and limits combat, remote, expedition, hard, and extreme offers for variety. Active Quest Capacity A quest can limit how many quests the same player may already have active when they try to start it: max active quests is a total per player cap enforced by this candidate quest. max active by tag is checked only for keys also present in the candidate's metadata.tags; it counts active quests carrying the same normalized tag. 0 means no cap. These gates prevent a new start but do not cancel an already active quest. Generic Criterion Objectives Use a criterion objective when another gameplay system should advance a quest without adding a dedicated objective type. The namespaced criterion identifies the event, match contains exact string/number/boolean filters, and the normal item, entity selectors, location, dimension, and count fields add optional constraints. Built in criteria are villagerretaliation:crafted, villagerretaliation:smelted, villagerretaliation:entity interacted, villagerretaliation:damage dealt, and villagerretaliation:dimension changed. Their match keys include item, entity, damage type, from, and to as appropriate. Other mods can publish their own namespaced criteria through QuestCriterionApi.trigger. Supplying an item or entity lets datapacks reuse the standard item, entity type/tag, dimension, and location matching rules. Match data is an open string map, so integrations can evolve independently of this mod's quest schema. Built in quests use these families: Family Values Use group. Questline or umbrella grouping Catalog organization and related content searches role. story, side, request, tutorial Journal sections, offer priority, and onboarding activity. gather, deliver, combat, explore, build, trade, social, choice Multi select gameplay filters and player preferences destination. village, overworld, nether, end, plus optional remote Travel planning and dimension filters tier. early, mid, late, endgame World progression recommendations difficulty. easy, normal, hard, extreme Danger and complexity guidance commitment. quick, standard, expedition Expected time and travel commitment theme. community, defense, mystery, craftsmanship, exploration Curated collections and thematic search feature. branching, scene, encounter, recoverable Special presentation, tooling, and regression test coverage party. recommended, challenge Advisory group play guidance without changing completion scope pool. commission, daily, quest board Eligibility for rotating or curated offer pools Use the full prefix in JSON: For built in content: use exactly one group. , role. , tier. , difficulty. , and commitment. tag use exactly one primary destination; add destination.remote when travel is a material part of the quest use one or more activity. and theme. tags when multiple descriptions apply use feature. , party. , and pool. only when the corresponding capability or curation rule applies keep role.tutorial, activity.social, and feature.recoverable available for future content instead of applying them to unrelated quests Datapacks may add their own tags. Avoid redefining the documented built in prefixes with different meanings, and keep mechanics in their dedicated fields so metadata cannot drift from runtime behavior. Dialogue And Scenes Stage dialogue slots normally use these names: Slot When it appears offer Quest can be started reminder Quest is active but not ready turn in Objectives are complete already completed Player already completed a non repeatable quest unavailable Provider is known but availability gates fail inactive Accepted quest is paused by active conditions missing target, missing proof, locate failed Target/proof helper states Inline scenes stay inside the quest module. Use external or external scene only when the scene is large, shared, localized separately, or deliberately owned by another datapack resource. Objective Composition And Bonuses Stages use all completion by default. Set completion.mode to any when one predicate is enough, or at least with count for k of n goals. Both objective references and condition predicates in complete when participate in the count. Bonus outcomes are one shot per quest run and stage. Their claimed IDs are saved before actions run, so reconnects and reloads cannot duplicate rewards. A bonus supports the same all, any, and at least modes and can use any normal action type. Transition Rules Keep each response to one transition source. Pick one of: direct response fields such as next, stage, scene, complete, abandon, or fail a transition object with stage, scene, response, complete, abandon, or fail a transition action such as quest transition Do not combine direct transition fields with a transition action on the same response. Put side effects, such as set variable, notification, or reputation, in actions, then put the single stage or scene move in transition. fail and abandon are different terminal outcomes. Failure records FAILED, runs only lifecycle.on fail, stores a normalized failure code and time, and never grants completion rewards or increments completion/abandonment counts. Voluntary abandonment records ABANDONED (or CONSUMED when authored that way) and runs only lifecycle.on abandon. Prerequisites And Restart Rules Use prerequisite cooldown (or its ticks, seconds, and days forms) when a follow up quest should wait after its prerequisites are completed. The cooldown is checked against every prerequisite, so the quest unlocks only after the most recently completed prerequisite has aged past the configured duration. Put every required quest in availability.prerequisites. The list is ordered for journal/debug presentation and every entry must be completed. metadata.parent remains a singular compatibility and organization field for older content. Quest module v2 also carries the full runtime rule set used by v1. Put active state gates, expiration policy, and branch exclusion under availability: complete when may mix objective references with condition predicates. Objective UI also accepts tracker complete text and tracker complete text key, and reward memory events retain memory scope. The v1 to v2 migration tool preserves these rules, stage tracker steps, and objective completion copy. Definition Revisions And Live Save Migration Increment metadata.revision whenever an update changes stage or objective identity for a quest that players may already have active. The migration policy is applied once when a save first sees the newer revision and the result is persisted for audit/debug output. Policies are: keep: remap aliases, retain progress, and safely move to the entry stage only if the saved stage no longer exists reset stage: remap aliases, then clear objective and bonus progress owned by the current stage restart: keep the quest run and provider binding but clear objective, target, and bonus progress and return to the entry stage fail: terminate the active quest with a revision specific failure code and run the normal failure integrations Legacy saves without a stored revision adopt the current revision without destructive migration. A datapack rollback never re applies an older revision over a newer persisted one. For v1 files, put revision and migration at the quest root; the v1 to v2 migration tool moves them into metadata. Failed quests can restart only when repeatable is true. max starts, max completions, provider locking, and completion scope still apply. Failure does not consume the quest by itself. Abandoned quests continue to follow abandonment, abandonment cooldown, and consume on abandonment. Missing Providers And Rebind Active progress remains in the journal using the saved provider name, profession, location, and UUID when the live villager is gone. The journal's Abandon quest action works without the live provider. If abandonment or expiration has an authored lifecycle hook, the runtime persists that event instead of dropping its provider bound actions. It replays the event once when the original provider is live again, or immediately after an operator supplies a compatible replacement. Turning in through another matching provider is allowed only with cross villager compatible: true. The runtime never chooses a nearby villager automatically. Operators can explicitly repair a missing binding with: The command refuses a rebind while the current provider is live, verifies the provider type and authored filters, retains the previous snapshot in save history, and reports the accepted or rejected audit result. A terminal quest can be rebound only while it has deferred lifecycle work. The rebind consumes that work after one dispatch without reopening the quest. The debug inspector lists pending lifecycle events alongside provider history. Branch Example This module records a route choice, moves to the chosen stage, and completes from either branch. Random Item Objectives The existing fixed selector remains unchanged: To choose one required item when a quest run starts, set selection to random and provide a non empty items array. A string beginning with expands the current item tag: Entries may be weighted. The default weight is 1: Weights select an entry first. If the selected entry is a tag, one item is then chosen uniformly from that tag. A large tag therefore has the same entry probability as a one item tag with the same weight. The resolved concrete item is stored in quest saved data and is not rerolled by reloads, reconnects, restarts, tracker refreshes, or objective checks. A new repeatable quest run resolves a new item. {objective item} and {objective item id} display the resolved name and ID in objective tracker text, UI text, and quest dialogue. Item components, durability, enchantment, custom data, and nbt requirements apply to the resolved item exactly as they do to a fixed item. Structure Target Example Root target fields define a structure search, discovery radius, and proof item. Stages can combine a visit objective with a proof item objective. An item check objective can add components, durability, custom data, or nbt to its item selector. These combine with the existing enchantment and durability percentage requirements. Root proof items use the prefixed forms proof item components, proof item durability, proof item custom data, and proof item nbt. Progress counting, hand in consumption, tracker synchronization, tooltips, slot highlights, held item glow, and dropped item outlines all use the same requirements; advanced objectives no longer highlight a nonmatching variant of the same item ID. Forced Or External Scene Example Use external scenes when another file owns a long conversation. Use forced dialogue actions when the quest needs an event driven locked scene. These actions need live player and provider context. If the quest giver is unloaded, the runtime records diagnostics and waits until it can safely run the live action. The external scene above can live in data/my pack/dialogue trees/en us/quests/storm warning.json: The forced quest scene above can live in data/my pack/forced dialogue/quests/lost civilization/storm warning.json: Quest Trigger Arbitration And Payloads Quest events can react to player tick, proximity, started, progress, mob kill, block break, block place, block interact, memory event, gift, trade, reputation, criterion, stage changed, completed, failed, abandoned, and expired. The dispatcher snapshots the canonical event, dispatch stage, game time, player/provider identity, position, and scalar payload values. Objective payloads therefore remain available to conditions and actions for that dispatch instead of being reduced to a bare event name. See Trigger Payload Conditions JSON Reference.md trigger payload conditions for the query shape. Matching triggers are grouped by priority. chance gates each trigger, weight orders surviving triggers inside a tier, and weight: 0 disables a trigger. Every selected trigger normally runs; after a successfully executed exclusive: true trigger, lower priority work stops. cooldown and its duration suffixes are stored per trigger and quest run. repeatable: false permits one successful execution for that run. Localization Inline text is a fallback. Use key fields when you want datapack localized text: Dialogue slots and scenes also accept text key, label key, and keyed lines where the generated schema lists them. Put keyed text in normal dialogue message files under data/ /dialogue/ /.../messages/ .json. Capabilities And Live Context Conditions and actions come from the generated quest registries. The datapack builder reads tools/datapack builder/quest registry metadata.json. The Node validator and Java schema generator use the same runtime metadata. Some registry entries need live entities: provider live conditions, such as villager equipment or live mood checks, need the quest giver loaded player live actions, such as notifications, forced dialogue, loot, XP, and reputation changes, need a player context provider live actions, such as forced dialogue and gossip, need the issuing villager loaded Prefer saved state conditions for active quest gates that must continue while the villager is unloaded. Use live context actions from events only when the event is expected to run near the player and provider. Diagnostics And Trace Commands Available diagnostic commands: Use inspect for saved state, issuer context, target context, repeat rules, objective counters, current stage, and fact values. Use trace for indexed trigger dispatch, condition traces, action diagnostics, and bounded recent events. Legacy V1 Compatibility V1 quest JSON remains supported when the file has no schema: \"villagerretaliation:quest/v2\". V1 fields such as display, offer, top level objectives, rules, tracker, triggers, and separate dialogue trees still load through the compatibility adapter. Legacy override rules still apply: a higher priority datapack can replace a built in quest by writing the same quest id a v2 module can replace a v1 quest with the same id old dialogue tree resources under data/ /dialogue trees/ /quests/... still work remove and replace on dialogue trees still remove or replace legacy/extracted scenes Do not delete v1 resources just because v2 exists. Migrate intentionally, validate the generated v2 file, and keep any external dialogue tree only when it is still needed. Extraction Guidance Start with one v2 quest file. Extract only when the file becomes hard to maintain: Use the quest module for quest state, stages, objective readiness, rewards, transitions, and short scenes. Use external dialogue trees for long authored branches or shared localization. Use forced dialogue only for event driven locked scenes outside the normal Talk flow.",
      "headings": [
        {
          "level": 2,
          "title": "Paths"
        ...
        {
          "level": 2,
          "title": "Active Quest Capacity"
        },
        {
          "level": 2,
          "title": "Generic Criterion Objectives"
        },
        {
          "level": 2,
        ...
        {
          "level": 2,
          "title": "Objective Composition And Bonuses"
        },
        {
          "level": 2,
        ...
        {
          "level": 2,
          "title": "Definition Revisions And Live-Save Migration"
        },
        {
          "level": 2,
        ...
        {
          "level": 2,
          "title": "Quest Trigger Arbitration And Payloads"
        },
        {
          "level": 2,
          "title": "Localization"
    ...
    },
    {
      ...
      "source": "wiki/dev/content/Dialogue-And-Quests.md",
      "sourceKind": "wiki",
      "group": "Quests & Scenes",
      "icon": "workflow",
    ...
    },
    {
      ...
      "source": "wiki/dev/content/Quest-Scenes.md",
      "sourceKind": "wiki",
      "group": "Quests & Scenes",
      "icon": "clapperboard",
    ...
    },
    {
      ...
      "source": "wiki/dev/content/Quest-Runtime-Roadmap.md",
      "sourceKind": "wiki",
      "group": "Quests & Scenes",
      "icon": "milestone",
    ...
    },
    {
      "slug": "gifts",
      "file": "Gifts.md",
      "source": "wiki/dev/content/Gifts.md",
      "sourceKind": "wiki",
    ...
    },
    {
      ...
      "source": "wiki/dev/content/Pacification.md",
      "sourceKind": "wiki",
      "group": "Economy & Progression",
      "icon": "hand-coins",
      "title": "Pacification",
    ...
    },
    {
      ...
      "source": "wiki/dev/content/Profession-Loot.md",
      "sourceKind": "wiki",
      "group": "Economy & Progression",
      "icon": "package",
    ...
    },
    {
      ...
      "source": "wiki/dev/content/Currency-And-Item-Text.md",
      "sourceKind": "wiki",
      "group": "Economy & Progression",
      "icon": "coins",
    ...
    },
    {
      "slug": "duel-kits",
      "file": "Duel-Kits.md",
      "source": "wiki/dev/content/Duel-Kits.md",
      "sourceKind": "wiki",
      "group": "Economy & Progression",
      "icon": "swords",
      "title": "Duel Kits",
    ...
    },
    {
      ...
      "source": "wiki/dev/content/Skill-Trades.md",
      "sourceKind": "wiki",
      "group": "Economy & Progression",
      "icon": "badge-percent",
      "title": "Skill Trades",
    ...
    },
    {
      ...
      "source": "wiki/dev/content/Sell-Box-And-Daily-Market.md",
      "sourceKind": "wiki",
      "group": "Economy & Progression",
      "icon": "store",
      ...
      "text": "Sell Box And Daily Market The Sell Box is a one slot village market. Put a saleable stack in the slot and press Sell ; the box uses that village's price for the day and sells the whole stack in one transaction. If you insert a second valid stack while the slot is occupied, the first stack sells and the new one becomes pending. The box keeps the exact balance, including fractions. Collect gives the player as many whole currency items as will fit and leaves the remainder in the box. Automation follows the same rules as the screen. Hoppers and other item handlers insert through the top or sides and extract whole currency items from the bottom. Assigned output couriers can deliver sale items, and supply couriers can collect the finished currency. None of these routes bypasses the stack checks described below. Where Price Files Go Put sell price files here: The namespace and path become the definition ID. A file at data/my pack/sell prices/coal.json creates my pack:coal. Use Explicit Rates For New Packs New definitions should put prices in a rates array. Each entry is one exchange rate family, so its item count always stays paired with its currency count. Before demand and supply adjustments, 15 coal are worth one currency item. A stack of 30 starts from a value of two. Either side can be a fixed positive integer or an inclusive min and max range: Here, each village can choose from the generated rates of 5, 6, 7, or 8 logs per currency. The choice is stable for that village and day; reloading does not reroll it. A currency range works the same way: When both sides of one rate are ranges, every combination belongs to that one family. Equivalent fractions are collapsed before the daily choice is made. Discrete Prices Stay Discrete Use separate rate entries when only a few exact prices should be possible: That produces 10, 15, or 25 currency per cocktail. It does not quietly fill in 11 through 24. There is no index pairing between separate item count and currency count arrays; the rate object is the association. Different shapes can live beside each other: Existing beta.13 Files Still Work The older top level pair remains supported: It loads as one rate with the same two counts. Keep old packs as they are if they are working, but prefer rates when writing something new. A file cannot use rates and the legacy top level pricing fields together; the server reports the conflict and skips that definition. Items And Tags item accepts a registered item ID or a prefixed item tag. One tag definition applies to every current member, including modded members, so a large tag does not need one JSON file per item. Tags are resolved again on /reload. If market group is omitted, an item definition defaults to the item ID and a tag definition defaults to the tag ID without . Unknown or empty tags are reported and skipped. Matching Item Components Use components when two stacks of the same item should have different prices. Values are decoded with the registered Minecraft 1.21.1 data component codec, including component types added by other mods. For example, a level six Miner's Star from Kaleidoscope Tavern can have its own price (for pink lol): Exact values use the component's normal codec representation. Numeric component values can use inclusive ranges: A range can have only min or only max. Ranges are rejected for components whose decoded value is not numeric. For NBT like custom item data, use the vanilla minecraft:custom data component: Custom data objects use subset matching. The stack must contain the required keys and values, but unrelated custom data does not prevent a match. Invalid component IDs, malformed codec values, and nonnumeric range mistakes produce a datapack diagnostic and skip the bad definition instead of breaking the reload. Exact Component Shorthand For a specific item with exact component values, vanilla item syntax is also accepted: Minecraft's own item parser handles this form. Use the structured components object for ranges, custom data predicates, and tag selectors. Remaining Durability durability means durability remaining, not raw damage: min: 250 means at least 250 durability remains. max: 25 means 25 or less remains. You can provide both bounds. Non damageable items never match a durability predicate, and negative, reversed, or impossible conditions are reported during reload. Overlapping Definitions And Priority Several definitions can target the same base item. This is useful for a broad fallback plus a component specific price: The server first gathers item and tag selectors that include the stack, then checks components and durability. The matching definition with the highest priority wins. The default priority is 0; having more predicates does not raise it automatically. Equal priority ties are deterministic: the later sorting definition ID wins, matching the old beta.13 conflict behavior. If the predicates can overlap, reload diagnostics ask you to set an explicit priority rather than leaving that tie accidental. Fields Field Required Meaning item Yes Registered item ID, exact item component shorthand, or prefixed item tag. rates New files One or more associated item count and currency count families. item count + currency count Legacy alternative Backward compatible top level pair, treated as one rate. Do not combine it with rates. components No Exact codec backed values or numeric min/max predicates. durability No Remaining durability min, max, or both. priority No Explicit winner among matching definitions. Defaults to 0. market group No Demand and supply group shared by related definitions. enabled No Set to false to disable a lower pack's resource at the same path. Count values must be positive. item count cannot exceed 256, each count range can contain at most 256 values, a definition can contain at most 256 rates, and total generated combinations are capped. These limits keep a malformed pack from creating an enormous candidate list. The Village Market Still Works The Same Way Predicates decide whether a stack is eligible and which definition wins. After that selection, the existing market does the rest: resolve the local village choose the village's daily demand band select one candidate base rate deterministically apply that village's current supply pressure calculate the exact payout record the completed sale's pressure market group is still the key that makes related goods share demand and supply pressure. Different villages keep separate pressure. Adding a component predicate does not create a second economy. The configured primary currency and accepted currency tags remain unsaleable, even if a sell price file targets them. Add, Replace, Or Disable A new resource path adds a definition. Reusing the same namespace and path in a higher priority datapack replaces that resource. To disable a lower pack's file, replace the same path with: Invalid ranges, selectors, components, and priorities are reported during reload. A bad entry is skipped; it does not crash the server. Built In Price Basis Built in definitions are based on direct Minecraft 1.21.1 villager and wandering trader offers. They do not copy live trade demand, reputation discounts, mod added offers, or secondary recipe inputs. The village market applies its own daily demand and supply pressure after choosing the matching definition.",
      "headings": [
        {
          "level": 2,
          "title": "Where Price Files Go"
        },
        {
          "level": 2,
          "title": "Use Explicit Rates For New Packs"
        },
        {
          "level": 2,
          "title": "Discrete Prices Stay Discrete"
        },
        {
          "level": 2,
          "title": "Existing beta.13 Files Still Work"
        },
        {
          "level": 2,
        ...
        {
          "level": 2,
          "title": "Matching Item Components"
        },
        {
          "level": 2,
          "title": "Exact Component Shorthand"
        },
        {
          "level": 2,
          "title": "Remaining Durability"
        },
        {
          "level": 2,
          "title": "Overlapping Definitions And Priority"
        },
        {
          "level": 2,
          "title": "Fields"
        ...
        {
          "level": 2,
          "title": "The Village Market Still Works The Same Way"
        },
        {
          "level": 2,
    ...
    },
    {
      ...
      "source": "wiki/dev/content/Builder-Structures.md",
      "sourceKind": "wiki",
      "group": "Economy & Progression",
      "icon": "building-2",
    ...
    },
    {
      ...
      "source": "wiki/dev/content/Natural-Job-Armor.md",
      "sourceKind": "wiki",
      "group": "Economy & Progression",
      "icon": "shield",
    ...
    },
    {
      ...
      "source": "wiki/dev/content/Story-Discovery.md",
      "sourceKind": "wiki",
    ...
    },
    {
      ...
      "source": "wiki/dev/content/Villager-Names.md",
      "sourceKind": "wiki",
      "group": "World & Identity",
      "icon": "user-round",
    ...
    },
    {
      ...
      "source": "wiki/dev/content/Village-Names.md",
      "sourceKind": "wiki",
      "group": "World & Identity",
      "icon": "landmark",
      "title": "Village Names",
    ...
    },
    {
      ...
      "source": "wiki/dev/content/Player-Raids.md",
      "sourceKind": "wiki",
      "group": "World & Identity",
      "icon": "flag",
      "title": "Player Raids",
    ...
    },
    {
      ...
      "source": "wiki/dev/content/Generated-Containers.md",
      "sourceKind": "wiki",
      "group": "World & Identity",
      "icon": "package-search",
    ...
    },
    {
      ...
      "source": "wiki/dev/content/Resource-Pack-Models.md",
      "sourceKind": "wiki",
    ...
    },
    {
      ...
      "source": "wiki/dev/content/Pack-Format-Changes.md",
      "sourceKind": "wiki",
      "group": "Reference",
      "icon": "history",
    ...
    },
    {
      ...
      "source": "wiki/dev/content/quest-scene-runtime.md",
    ...
    },
    {
      ...
      "source": "wiki/dev/content/tracked-villages.md",
      "sourceKind": "implementation",
      "group": "Runtime Internals",
      "icon": "map-pinned",
  ...
  "examples": [
    ...
    },
    {
      ...
      "page": "json-reference",
      "pageTitle": "JSON Reference",
      "section": "Trigger Payload Conditions",
      "level": "Advanced",
      "language": "json",
      "code": "{\n  \"type\": \"trigger_payload\",\n  \"events\": [\"criterion\"],\n  \"all\": {\n    \"criterion\": [\"villagerretaliation:crafted\"],\n    \"criterion_item\": [\"minecraft:iron_sword\"]\n  },\n  \"not\": {\n    \"dimension\": [\"minecraft:the_nether\"]\n  }\n}"
    },
    ...
    {
      "id": "json-reference-example-27",
      "page": "json-reference",
      "pageTitle": "JSON Reference",
      "section": "Shared Text Variants And Usage",
      "level": "Advanced",
      "language": "json",
      "code": "{\n  \"variants\": [\n    {\n      \"id\": \"storm\",\n      \"text\": \"The bridge is unsafe in this weather.\",\n      \"priority\": 10,\n      \"chance\": 0.8,\n      \"weight\": 3,\n      \"conditions\": [\n        { \"type\": \"weather\", \"state\": \"thunder\" }\n      ],\n      \"usage\": {\n        \"cooldown_days\": 1,\n        \"max_uses\": 3,\n        \"anti_repeat\": true,\n        \"scope\": \"player_villager\"\n      },\n      \"metadata\": {\n        \"topic\": \"bridge_warning\",\n        \"routing_tags\": [\"route.warning\"],\n        \"anti_repeat_groups\": [\"rotation.bridge\"]\n      }\n    }\n  ]\n}"
    },
    {
      "id": "json-reference-example-28",
      "page": "json-reference",
      "pageTitle": "JSON Reference",
    ...
    {
      "id": "dialogue-example-11",
      "page": "dialogue",
      "pageTitle": "Dialogue",
      "section": "Rich Text Variants",
      "level": "Advanced",
      "language": "json",
      "code": "{\n  \"id\": \"my_pack.line.market_rumor\",\n  \"request\": \"story\",\n  \"metadata\": {\n    \"topic\": \"market_rumors\",\n    \"tags\": [\"dialogue.ambient\", \"tone.friendly\"],\n    \"routing_tags\": [\"route.market\"],\n    \"anti_repeat_groups\": [\"rotation.market\"]\n  },\n  \"variants\": [\n    {\n      \"id\": \"rain\",\n      \"text\": \"Rain makes every market promise sound urgent.\",\n      \"priority\": 10,\n      \"weight\": 3,\n      \"chance\": 0.75,\n      \"conditions\": [\n        { \"type\": \"weather\", \"state\": \"rain\" }\n      ],\n      \"usage\": {\n        \"cooldown_days\": 1,\n        \"anti_repeat\": true,\n        \"scope\": \"player_villager\"\n      }\n    },\n    {\n      \"id\": \"clear\",\n      \"text_key\": \"my_pack.market_rumor.clear\",\n      \"weight\": 1\n    }\n  ]\n}"
    },
    {
      "id": "dialogue-example-12",
      "page": "dialogue",
      "pageTitle": "Dialogue",
      "section": "Durable Usage Rules",
      "level": "Advanced",
      "language": "json",
      "code": "{\n  \"id\": \"my_pack.line.first_warning\",\n  \"request\": \"question\",\n  \"text\": \"I will explain this once.\",\n  \"usage\": {\n    \"once\": true,\n    \"cooldown_seconds\": 30,\n    \"anti_repeat\": true,\n    \"scope\": \"player_villager\"\n  }\n}"
    },
    {
      "id": "dialogue-example-13",
      "page": "dialogue",
      "pageTitle": "Dialogue",
      "section": "Dialogue Frequency Tuning",
      "level": "Advanced",
      "language": "json",
      "code": "{\n  \"schema\": \"villagerretaliation:dialogue_tuning/v1\",\n  \"values\": {\n    \"story_hint.vague_chance\": 0.2,\n    \"reputation.joke.neutral_chance\": 0.35\n  }\n}"
    },
    ...
    },
    {
      ...
      "page": "dialogue-trees",
      "pageTitle": "Dialogue Trees",
      "section": "Conditions, Metadata, And Rich Text",
      "level": "Advanced",
      "language": "json",
      "code": "{\n  \"metadata\": {\n    \"topic\": \"old_roads\",\n    \"routing_tags\": [\"route.story\"],\n    \"anti_repeat_groups\": [\"rotation.old_roads\"],\n    \"quest\": \"my_pack:road_ledger\"\n  },\n  \"conditions\": [\n    { \"type\": \"dimension\", \"dimension\": \"minecraft:overworld\" }\n  ]\n}"
    },
    ...
    },
    {
      ...
      "page": "forced-dialogue",
      "pageTitle": "Forced Dialogue",
      "section": "Shared Conditions And Rich Variants",
      "level": "Advanced",
      "language": "json",
      "code": "{\n  \"id\": \"my_pack.retaliation.warning\",\n  \"trigger\": \"retaliation_started\",\n  \"output\": { \"mode\": \"chat\" },\n  \"lines\": [\n    {\n      \"id\": \"storm\",\n      \"text\": \"Even the storm is quieter than your mistakes.\",\n      \"priority\": 10,\n      \"chance\": 0.5,\n      \"weight\": 2\n    },\n    {\n      \"id\": \"fallback\",\n      \"text_key\": \"my_pack.retaliation.warning\",\n      \"weight\": 1\n    }\n  ],\n  \"conditions\": [\n    { \"type\": \"reputation\", \"levels\": [\"hostile\", \"despised\", \"feared\"] }\n  ]\n}"
    },
    ...
    },
    {
      ...
      "page": "quests",
      "pageTitle": "Quests",
      "section": "Generic Criterion Objectives",
      "level": "Advanced",
      "language": "json",
      "code": "{\n  \"id\": \"forge_blades\",\n  \"type\": \"criterion\",\n  \"criterion\": \"villagerretaliation:crafted\",\n  \"item\": \"minecraft:iron_sword\",\n  \"match\": {\n    \"item\": \"minecraft:iron_sword\"\n  },\n  \"count\": 3\n}"
    },
    ...
    },
    {
      ...
      "page": "quests",
      "pageTitle": "Quests",
      "section": "Objective Composition And Bonuses",
      "level": "Advanced",
      "language": "json",
      "code": "{\n  \"complete_when\": [\"secure_gate\", \"rescue_smith\", \"recover_ledger\"],\n  \"completion\": { \"mode\": \"at_least\", \"count\": 2 },\n  \"bonuses\": [\n    {\n      \"id\": \"save_everyone\",\n      \"when\": [\"secure_gate\", \"rescue_smith\", \"recover_ledger\"],\n      \"mode\": \"all\",\n      \"actions\": [\n        { \"type\": \"experience\", \"amount\": 100 },\n        { \"type\": \"reputation\", \"amount\": 5 }\n      ]\n    }\n  ]\n}"
    },
    ...
    },
    {
      ...
      "page": "quests",
      "pageTitle": "Quests",
      "section": "Definition Revisions And Live-Save Migration",
      "level": "Advanced",
      "language": "json",
      "code": "\"metadata\": {\n  \"revision\": 3,\n  \"migration\": {\n    \"active_policy\": \"keep\",\n    \"stage_aliases\": {\n      \"find_ruins\": \"survey_ruins\"\n    },\n    \"objective_aliases\": {\n      \"find_ruins.old_map\": \"survey_ruins.map_fragment\"\n    }\n  }\n}"
    },
    {
      "id": "quests-example-10",
      "page": "quests",
      "pageTitle": "Quests",
      "section": "Definition Revisions And Live-Save Migration",
      "level": "Advanced",
    ...
    {
      "id": "quests-example-12",
      "page": "quests",
      "pageTitle": "Quests",
      "section": "Random Item Objectives",
      "level": "Advanced",
      "language": "json",
      "code": "{\n  \"type\": \"item_check\",\n  \"item\": \"minecraft:bread\",\n  \"count\": 8\n}"
    },
    {
      "id": "quests-example-13",
      "page": "quests",
      "pageTitle": "Quests",
      "section": "Random Item Objectives",
      "level": "Advanced",
    ...
    {
      "id": "quests-example-19",
      "page": "quests",
      "pageTitle": "Quests",
      "section": "Quest Trigger Arbitration And Payloads",
      "level": "Advanced",
      "language": "json",
      "code": "{\n  \"events\": [\n    {\n      \"id\": \"crafted_blade_reaction\",\n      \"event\": \"criterion\",\n      \"stages\": [\"forge\"],\n      \"priority\": 20,\n      \"chance\": 0.75,\n      \"weight\": 3,\n      \"exclusive\": true,\n      \"cooldown_seconds\": 30,\n      \"repeatable\": true,\n      \"conditions\": [\n        {\n          \"type\": \"trigger_payload\",\n          \"all\": {\n            \"criterion\": [\"villagerretaliation:crafted\"],\n            \"criterion_item\": [\"minecraft:iron_sword\"]\n          }\n        }\n      ],\n      \"actions\": [\n        {\n          \"type\": \"notification\",\n          \"trigger\": \"quest.updated\",\n          \"text\": \"The smith noticed your finished blade.\"\n        }\n      ]\n    }\n  ]\n}"
    },
    {
      "id": "quests-example-20",
      "page": "quests",
      "pageTitle": "Quests",
    ...
    },
    {
      ...
      "section": "Use Explicit Rates For New Packs",
      "level": "Starter",
      "language": "json",
      "code": "{\n  \"item\": \"minecraft:coal\",\n  \"rates\": [\n    {\n      \"item_count\": 15,\n      \"currency_count\": 1\n    }\n  ],\n  \"market_group\": \"villagerretaliation:fuel\"\n}"
    },
    ...
    },
    {
      ...
      "page": "sell-box-and-daily-market",
      "pageTitle": "Sell Box And Daily Market",
      "section": "Discrete Prices Stay Discrete",
      "level": "Advanced",
      "language": "json",
      "code": "{\n  \"item\": \"#vr-food-sellbox:cocktails\",\n  \"rates\": [\n    {\n      \"item_count\": 1,\n      \"currency_count\": 10\n    },\n    {\n      \"item_count\": 1,\n      \"currency_count\": 15\n    },\n    {\n      \"item_count\": 1,\n      \"currency_count\": 25\n    }\n  ],\n  \"market_group\": \"vfs:cocktails\"\n}"
    },
    {
      "id": "sell-box-and-daily-market-example-5",
      "page": "sell-box-and-daily-market",
      "pageTitle": "Sell Box And Daily Market",
      "section": "Discrete Prices Stay Discrete",
      "level": "Advanced",
    ...
    {
      "id": "sell-box-and-daily-market-example-6",
      "page": "sell-box-and-daily-market",
      "pageTitle": "Sell Box And Daily Market",
      "section": "Existing beta.13 Files Still Work",
      "level": "Advanced",
      "language": "json",
      "code": "{\n  \"item\": \"minecraft:charcoal\",\n  \"item_count\": {\n    \"min\": 12,\n    \"max\": 20\n  },\n  \"currency_count\": 1\n}"
    },
    {
      "id": "sell-box-and-daily-market-example-7",
      "page": "sell-box-and-daily-market",
      "pageTitle": "Sell Box And Daily Market",
      "section": "Matching Item Components",
      "level": "Advanced",
      "language": "json",
      "code": "{\n  \"item\": \"kaleidoscope_tavern:miners_star\",\n  \"components\": {\n    \"kaleidoscope_tavern:brew_level\": 6\n  },\n  \"priority\": 10,\n  \"rates\": [\n    {\n      \"item_count\": 1,\n      \"currency_count\": 15\n    }\n  ],\n  \"market_group\": \"kaleidoscope_tavern:miners_stars\"\n}"
    },
    {
      "id": "sell-box-and-daily-market-example-8",
      "page": "sell-box-and-daily-market",
      "pageTitle": "Sell Box And Daily Market",
      "section": "Matching Item Components",
      "level": "Advanced",
    ...
    {
      "id": "sell-box-and-daily-market-example-9",
      "page": "sell-box-and-daily-market",
      "pageTitle": "Sell Box And Daily Market",
      "section": "Matching Item Components",
      "level": "Advanced",
    ...
    {
      "id": "sell-box-and-daily-market-example-10",
      "page": "sell-box-and-daily-market",
      "pageTitle": "Sell Box And Daily Market",
      "section": "Exact Component Shorthand",
      "level": "Advanced",
      "language": "json",
      "code": "{\n  \"item\": \"kaleidoscope_tavern:miners_star[kaleidoscope_tavern:brew_level=6]\",\n  \"rates\": [\n    {\n      \"item_count\": 1,\n      \"currency_count\": 15\n    }\n  ]\n}"
    },
    {
      "id": "sell-box-and-daily-market-example-11",
      "page": "sell-box-and-daily-market",
      "pageTitle": "Sell Box And Daily Market",
      "section": "Remaining Durability",
      "level": "Advanced",
      "language": "json",
      "code": "{\n  \"item\": \"#example:valuable_pickaxes\",\n  \"components\": {\n    \"example:quality\": {\n      \"min\": 3\n    }\n  },\n  \"durability\": {\n    \"min\": 250\n  },\n  \"rates\": [\n    {\n      \"item_count\": 1,\n      \"currency_count\": 5\n    },\n    {\n      \"item_count\": 1,\n      \"currency_count\": 8\n    },\n    {\n      \"item_count\": 1,\n      \"currency_count\": 12\n    }\n  ],\n  \"market_group\": \"example:tools\"\n}"
    },
    {
      "id": "sell-box-and-daily-market-example-12",
      "page": "sell-box-and-daily-market",
      "pageTitle": "Sell Box And Daily Market",
      "section": "Overlapping Definitions And Priority",
      "level": "Advanced",
      "language": "json",
      "code": "{\n  \"item\": \"kaleidoscope_tavern:miners_star\",\n  \"rates\": [\n    {\n      \"item_count\": 1,\n      \"currency_count\": 5\n    }\n  ]\n}"
    },
    {
      "id": "sell-box-and-daily-market-example-13",
      "page": "sell-box-and-daily-market",
      "pageTitle": "Sell Box And Daily Market",
      "section": "Overlapping Definitions And Priority",
      "level": "Advanced",
    ...
    {
      "id": "sell-box-and-daily-market-example-14",
      "page": "sell-box-and-daily-market",
      "pageTitle": "Sell Box And Daily Market",
  ...
  "packs": [
    ...
    },
    {
      ...
      "files": [
        ...
        "data/example/sell_prices/miners_star.json",
