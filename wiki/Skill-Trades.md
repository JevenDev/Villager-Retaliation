...
When one or more queued refreshes become ready for that same player, the villager opens a data-driven ready follow-up (or falls back to chat) so packs can customize how the restocked results are presented before trading resumes. Multiple nearby villagers with ready requests can join the same locked forced-dialogue flow as interjections instead of opening separate screens.
...
- Ready follow-up lines use `trade_refresh.ready`, with replacements such as `{restocked_summary}`, `{trade_items}`, `{trade_count}`, and `{trade_word}`. Ready interjections use `trade_refresh.ready_interjection`, or `trade_refresh.ready_theft_interjection` when they interrupt an active container confrontation.
- The forced-dialogue option labels and responses use `trigger: "trade_refresh"` entries in `data/villagerretaliation/forced_dialogue/`, including `trade_refresh.ready_options` for ready-state prompts.
...
The default balance is a 2 Minecraft day wait and a 3 Minecraft day cooldown. Entry metadata can override both. Cooldown starts when the order is accepted, so the same player must wait before placing another Special Order with the same villager. Extra request costs are supported by metadata and are only consumed when `specialOrderExtraCostEnabled` is true and the request passes validation. Payment is taken when the request is accepted, not when the order completes.
...
| `cooldown_days` | integer | config value | Minecraft days after request acceptance before the same player can place another order with that villager. |
