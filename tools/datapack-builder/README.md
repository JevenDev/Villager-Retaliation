...
The Quests tab authors `villagerretaliation:quest/v2` modules as single-file JSON entries. It loads the generated quest registry metadata and JSON schema from `quest-registry-metadata.json` and `quest-v2.schema.json`, offers inline versus external scene references, warns about duplicate export paths and conflicting response transitions, and preserves unknown imported fields when round-tripping module JSON.
