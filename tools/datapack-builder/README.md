...
The builder is a static site. It emits the selected Villager Retaliation pack version's wiki paths for dialogue, forced dialogue events, chat-only event lines, notifications, gifts, pacification payments, story discovery, preset names, and `pack.mcmeta`, then exports them as a datapack zip with root-level `pack.mcmeta`.
...
For beta.11 packs, the builder writes `small_talk` for normal Talk menu conversation. Older imported packs that use `chat` should be updated to `small_talk` before export. Use `_chat` forced-dialogue triggers such as `retaliation_started_chat` when an event should send a villager-styled chat line without opening a forced conversation.
