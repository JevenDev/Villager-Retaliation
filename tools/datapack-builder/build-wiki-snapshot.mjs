...
const liveWikiRoot = path.resolve(root, "../../wiki/dev/content");
const liveWikiVersion = "1.0.0-beta.13";
...
async function buildSnapshot() {
  ...
  const liveFiles = await readdir(liveWikiRoot, { withFileTypes: true });
  snapshot[liveWikiVersion] = {};
  for (const file of liveFiles.filter((entry) => entry.isFile() && entry.name.endsWith(".md")).sort(byName)) {
    const source = await readFile(path.join(liveWikiRoot, file.name), "utf8");
    snapshot[liveWikiVersion][file.name] = normalizeLineEndings(source);
  }

  return `// Generated by tools/datapack-builder/build-wiki-snapshot.mjs. Do not edit directly.\nvar VR_WIKI_SNAPSHOT = ${JSON.stringify(snapshot, null, 2)};\nwindow.VR_WIKI_SNAPSHOT = VR_WIKI_SNAPSHOT;\n`;
}
