window.VR_DEVELOPER_WIKI_DATA = {
  "version": "1.0.0-beta.13-pre-release.1",
  ...
  "pages": [
    ...
    },
    {
      ...
      "description": "The high-level runtime contract for typed actors, resumable scenes, and recovery.",
      ...
      "text": "Quest Runtime Roadmap The Quest Runtime Foundation makes lifecycle changes explicit, versioned, and recoverable. The next milestone should add narrative scene capabilities on top of that boundary without moving Minecraft side effects into the state machine core. Typed actors and bindings Add typed actor aliases for villagers, players, hostile entities, and extension defined actors. Persist actor bindings by alias, provider type, stable identity, and last known snapshot. Give each alias an authored replacement policy: fixed, operator rebindable, compatible replacement, or optional. Keep replacement history so recovery never silently rewrites narrative identity. Resumable scenes Treat the foundation's persisted, one shot lifecycle event replay as the minimum recovery layer; scene resumption must build on it without broadening an event into an untracked multi step workflow. Represent scene work as persisted, idempotent steps with stable step IDs. Resume safely after save/reload, chunk unload, disconnect, or actor replacement. Separate dialogue, movement, waits, encounter gates, and completion markers so a repeated tick cannot replay a completed side effect. Safe actions and encounters Add safe namespaced function actions with an allowlisted registration contract; do not expose arbitrary command strings. Add encounter control for registered spawn groups, objectives, cleanup policies, and recovery after partial execution. Require server authoritative validation and explicit capability declarations for every action. Extension APIs Provide registration APIs and stable descriptors for providers, objectives, actions, conditions, and triggers. Extensions should declare required live/snapshot capabilities, serialization, diagnostics, and idempotency behavior so tooling can export the same contract used by the runtime. Recovery contract Every persistent scene step should record its input binding version, started/completed markers, and follow up side effects. On reload or chunk return, the runtime should reconcile those markers before doing work, preserve readable data from newer versions, and stop with a precise diagnostic when recovery cannot be proven safe.",
      "headings": [
        {
          "level": 2,
          "title": "Typed actors and bindings"
        },
        {
          "level": 2,
          "title": "Resumable scenes"
        },
        {
          "level": 2,
          "title": "Safe actions and encounters"
        },
        {
          "level": 2,
    ...
    },
    {
      ...
      "text": "Pacification Pacification files decide which items can calm a hostile villager or wandering trader. Path Simple Example Example: Modded Currency Example: Profession Specific Cost Main Fields Field Use items or tags Which item ids or tags qualify count Exact payment min count / max count Randomized payment range professions Restrict the rule to specific villager professions priority Break ties between several matches name / plural name Better wording for spoken text and placeholders The spoken pacify line itself belongs in normal dialogue under a pacify/ folder.",
      "headings": [
        {
          "level": 2,
          "title": "Path"
        ...
        {
          "level": 2,
          "title": "Example: Modded Currency"
        },
        {
          "level": 2,
          "title": "Example: Profession-Specific Cost"
        },
        {
          "level": 2,
          "title": "Main Fields"
    ...
    },
    {
      ...
      "text": "Profession Loot Profession loot ties villager professions to Minecraft loot tables. Paths Rules: Referenced loot tables: Rule Example Loot Table Example Common Uses give a modded profession its own drops add a rare drop to an existing vanilla profession remove one built in profession rule by id Chance Values Use either: \"always\" \"rare\" \"very rare\" a fixed numeric chance such as 0.25 rare and very rare defer to the mod's config values.",
      "headings": [
        {
          "level": 2,
          "title": "Paths"
    ...
    },
    {
      ...
      "text": "Sell Box and Daily Market The Sell Box is a public one slot market container. Put a saleable stack in its slot and press Sell to convert the whole stack at today's price. Putting another valid stack into an occupied box sells the old stack first, as one transaction, and leaves the new stack pending. The box keeps an exact shared balance. Collect moves only whole primary currency items that fit in the player's inventory; any fractional remainder stays in the box. Hoppers and other item handlers insert through the top or sides and extract whole primary currency items from the bottom. An assigned output courier can deposit saleable items, while a courier can collect currency from assigned Supplies storage. Pending sale items are never exposed as courier supplies. Price definitions Add one JSON file per definition at: The resource path is the definition ID. A fixed price uses positive integer counts: Either count may instead be an inclusive range: item count and the maximum of an item count range may not exceed 256. Each range may contain at most 256 values. The unit price is currency count / item count. Every distinct reduced ratio in the configured ranges is a candidate. Each village selects one deterministically from the world seed, its village identity, the definition ID, and the global overworld day. A definition with multiple candidates does not repeat the same candidate on consecutive days within that village. market group is optional and defaults to the sold item's ID. Items in the same group share that village's daily demand band and accumulated supply pressure, so overrides of grouped built in items should repeat the built in group. Daily demand raises or lowers the base rate by group. Completed sales then add local supply pressure, progressively reducing later payouts in that village until the pressure recovers over subsequent days. Other villages maintain independent rates and pressure. The active currency item and every item matched by the configured currency tags are always unsaleable, even if a price file names them. Item matching ignores durability and components. Override, add, or disable Add a new resource path to add an item. Use the same namespace and resource path in a higher priority pack to replace a definition. Replace the same resource path with the following file to remove the lower priority definition: If two active definitions use different resource paths for the same item, the lexicographically later resource ID wins deterministically and the server reports a datapack diagnostic. Pack priority only controls replacement at the same resource path. Invalid ranges, unknown items, and unknown fields are also reported during reload. The built in pack contains definitions derived from the direct Minecraft 1.21.1 villager and wandering trader offers. Vanilla trade demand changes, reputation discounts, mod added trades, and standalone auxiliary inputs are not part of those base definitions; the village market's own daily demand and supply pressure multipliers are applied afterward.",
      "headings": [
        {
          "level": 2,
    ...
    },
    {
      ...
      "text": "Story Discovery Story discovery files define the structures and biomes villagers can talk about through share story. Paths Structure Example Biome Example Dialogue Example Main Fields Field Use structure / structures One or more structure ids biome / biomes One or more biome ids name Human readable display name radius Discovery radius for structure checks If name is omitted, the mod derives a readable name from the id path.",
      "headings": [
        {
          "level": 2,
          "title": "Paths"
    ...
    },
    {
      ...
      "text": "Quest Scene Runtime This document defines the durable contract between quest runs, scene instances, continuations, encounters, and cleanup. Definitive quest run identity A solo run ID is derived from the player UUID, quest resource ID, and persisted start count. It is stable for one run, different for every legitimate repeat, and cannot be replaced after allocation. A shared party quest uses the durable PartySharedQuestRecord.instanceId for every member, regardless of personal start history. Startup establishes identity before authored effects: 1. decide solo or shared party ownership; 2. allocate or recover the definitive run ID; 3. persist quest, provider, and run state; 4. enroll/link party members using that ID; 5. run stage entry actions; 6. dispatch STARTED triggers; 7. synchronize tracker/UI state. Duplicate start requests observe the already active progress and do not increment startCount. Owners and operation keys PLAYER: player identity plus the quest/run/operation key; repeats are separate. PARTY: one owner per party and shared quest run. Reuse merges enrolled participants and missing bindings but never replaces a fixed binding. QUEST INSTANCE: the definitive quest run UUID. Unrelated players cannot collide. WORLD: a global singleton for the dimension, scene, quest, and operation ID. It is intentionally not run scoped; authors must choose globally stable operation IDs. The operation index is rebuilt on load. Legacy owner aliases and compact terminal tombstones also participate in lookup. Wait for result continuations wait for result: false launches/reuses a scene and continues immediately. true records a non blocking continuation containing the scene ID, quest/run, player/provider, source pointer, compiled action snapshot, next action index, replacements, terminal result, and completion receipt. Success resumes remaining actions once. Failure and cancellation are recorded distinctly and do not execute success actions. Duplicate packets reuse the pending continuation. Offline players or unloaded providers leave it pending. A stale optional scene definition skips the optional launch; a required launch stops the sequence. Scene action batch cannot safely suspend and is rejected during compilation. Maintenance processes at most 16 continuations per tick. Deadlines Overall timeout zero is disabled for compatibility. Otherwise the absolute deadline is startGameTime + timeoutTicks with saturating overflow. Waiting scenes schedule the earlier of their step wake and overall deadline. Blocked scenes retain a deadline wake without polling. Reload reconstructs the same deadline; an overdue scene is processed once. Definition reload uses the current compiled timeout with the persisted start time. Shortening an elapsed timeout fires immediately; setting it to zero disables the overall deadline deterministically. Failure and cancellation policies All executor failures, encounter failures, actor policy failures, overall timeouts, quest terminal callbacks, and operator cancellations use one persisted transition service: FAIL SCENE: terminal failure; CANCEL SCENE: terminal cancellation; BLOCK FOR REPAIR: durable, explicitly repairable block; RUN FAILURE STEP: advance once to the current step's failure step. A missing failure step blocks with a focused diagnostic. Transition intent and the applied marker survive reload. Operator resume accepts only policy created repairable blocks. The typed quest transition scene step accepts exactly one of target stage/target: stage, target: complete, target: fail, or target: abandon. Mixed and unknown forms fail compilation. Quest terminal callbacks exclude the originating scene while still applying sibling cancellation policy. Cleanup lifecycle Scene terminal result and cleanup are orthogonal. A terminal scene keeps COMPLETED, FAILED, or CANCELLED while CleanupStatus moves from RUNNING to COMPLETE (or BLOCKED with a diagnostic/backoff). CLEANING UP is reserved for legacy state compatibility and is not the authoritative cleanup marker. Every terminal path queues cleanup once. Encounter cleanup removes/releases only owned entities, restores only blocks still matching the owned replacement, and preserves later player edits. Missing definitions retry after 1,200 ticks instead of spinning. Operator inspection exposes cleanup status, diagnostic, and retry time. Saves, legacy aliases, and exactly once records Scene save version 3 introduced explicit RunIdentityKind: QUEST RUN or LEGACY OWNER. This marker survives every save and keeps pre run ID PLAYER and QUEST INSTANCE operations reusable until terminal. Version 4 added terminal tombstones. Non idempotent effects use operation receipts with PREPARED, APPLIED, and COMPLETED states. Ambiguous prepared work blocks for repair. Continuations have their own completion receipt. Compaction never removes unresolved receipts, pending continuations/rewards, active encounters, blocked cleanup, or nonterminal scenes. Fully settled terminal scenes remain detailed for seven in game days, then compact incrementally into replay blocking tombstones containing the operation identity, result, times, and completed receipt IDs. Maintenance inspects at most 16 candidates every 200 ticks and retains up to 4,096 tombstones. Protected villagers and the downed state Protected villagers do not enter the normal death path after ordinary lethal damage. Final post mitigation damage is clamped in LivingDamageEvent.Pre, the villager remains at one health, and a persistent downed record stores the entry time, earliest recovery time, protection source diagnostics, data version, and the exact AI/pickup flags that must be restored. Party contracts, quest provider bindings, scene bindings, inventories, and hired work state are not removed. Protection is active when any of these sources applies: combat.allVillagersUseDownedState is enabled; the villager is in a vanilla or player raid and combat.raidVillagersUseDownedState is enabled; the villager has an active hired contract and combat.hiredVillagersUseDownedState is enabled; the villager has an active party contract and combat.partyVillagersUseDownedState is enabled; an active quest run from that exact provider UUID uses death protection: \"while active\"; that provider successfully started a quest using death protection: \"after start\"; an active scene binds the exact villager to an actor with lethal damage policy: \"downed\"; the entity has the permanent scoreboard tag villagerretaliation essential. After a protection source qualifies the villager, the player, mob/entity, and environmental damage toggles independently decide whether that lethal source may down them. Disabled source categories can finish an already downed villager with a lethal hit. Operator kill, void, and invulnerability bypassing sources always bypass protection. While downed, AI, navigation, attacks, work, follow behavior, pickup, trading, gifts, breeding, and dialogue are suspended. Repeated attacks still run ordinary hit consequences once but cannot reduce health. Nearby mobs targeting the villager are cleared on entry and once per second. The client receives transition packets and start tracking replay, then renders one of three stable whole body poses: sitting, lying on one side, or resting on hands and knees. Each variation also resizes the villager's hitbox and moves its name tag attachment to match the downed silhouette. Recovery requires the configured minimum duration, no nearby natural hostile or mob directly targeting the villager within downedThreatRadius, and downedQuietTicks of quiet. Health returns to downedRecoveryHealthPercent of maximum (at least one), prior AI/pickup flags are restored, and DOWNED scene bindings return to LIVE and wake their scenes. If the original protection expires while the villager is downed, recovery still finishes normally. When Second Wind is installed, every villager protected by this resolver is also exposed through Second Wind's player revive channel. VR still owns the state: there is no bleedout deadline, and a villager that is not manually revived continues through the normal quiet period recovery above. Compatible clients deterministically select sitting, side lying, hands and knees, or Second Wind's crawl posture for each new downed record. Ordinary unprotected villagers are not opted in. /kill (minecraft:generic kill), out of world damage, and damage tagged minecraft:bypasses invulnerability bypass protection. Direct entity discard/removal also remains permanent; void damage is allowed to kill so a protected villager cannot fall forever. Quest v2 provider contract: none is the default. while active applies only while at least one active run is bound to the exact provider UUID. after start writes the originating quest ID to the villager only after durable startup succeeds; offers and viewed dialogue do not protect, and the marker survives terminal quest states and reloads. Multiple quest IDs coexist without duplicate start side effects. Invalid values emit a focused diagnostic and fall back to none. Scene v1 actor contract: lethal damage policy defaults to normal. It prevents death only while the owning scene is active and is intentionally separate from the post death death policy. A downed binding persists as DOWNED; actor dependent steps block without polling until recovery wakes and resumes the scene.",
      "headings": [
        {
          "level": 2,
    ...
    },
    {
      ...
      "text": "Tracked Villages Tracked villages are server authoritative identities seeded by occupied village POIs. Each record has a UUID, generated or custom name, canonical alias, section footprint, resident roster, lifecycle, and last observed state. The footprint combines POI influence, tagged worldgen structure pieces, and connected tagged terrain. Data driven footprint support Two ordinary datapack tags control non POI coverage: villagerretaliation:village footprint is a worldgen/structure tag. It includes minecraft:village by default, so every vanilla plains, desert, savanna, snowy, and taiga village piece including road pieces is covered. villagerretaliation:village terrain is a block tag containing minecraft:dirt path by default. Sections containing these blocks extend the footprint only when they form a connected chain from the POI/structure footprint; unrelated paths elsewhere remain outside. Mods and modpacks can append structures or terrain blocks with normal replace: false tag files. Structure and terrain scans inspect loaded chunks only and do not force load worldgen. Allegiance rules Villagers and naturally created iron golems spawned inside a tracked footprint receive that village permanently. Newborn villagers born inside a village receive that village, regardless of their parents' homes. Outside a village, a baby inherits the first parent's known home; if neither parent has one, the baby is a Wanderer. Villagers spawned outside every tracked footprint are Wanderers. They are neutral to all villages and can be recruited against any village. A non party Wanderer who remains inside the same active village for 24,000 ticks settles there automatically. Leaving, changing villages, joining a party, or a backward game time change resets the settlement clock. Villagers who already have a home never change it merely by traveling or claiming a local bed or workstation. Party villagers never settle automatically. A party villager can adopt the current village only when ordered by a Revered or Royalty player in that same party; outside players cannot issue the order. Recruited foreign residents can fight another village without the target belonging to a party. Same party and same canonical village combat is always rejected. A merge therefore invalidates an older combat authorization immediately. Community retaliation begins only after damage lands and spreads only through the harmed resident's canonical village. Conversion preserves current version allegiance. Legacy v1 or missing data is classified lazily from the entity's position when it loads. An uncertain assignment remains pending across entity saves and retries until the surrounding chunks provide enough evidence to resolve it safely. Deliberate reassignment requires Revered individual trust and a repeated confirmation within 30 seconds. The party restriction above is applied before trust is considered. Connected POI footprints merge automatically only after three spaced observations show the same occupied POI connection. Terrain only or diagonal section contact cannot merge identities. Current footprints may shrink as evidence changes while historical coverage remains available for diagnostics. Empty records advance toward archival only while every footprint chunk is loaded and observed. After 72,000 observed ticks with no occupied source POI, the identity archives; rebuilding creates a new identity. The player facing Home topic answers direct questions about the villager's home and the current village. Technical assignment history remains available to allegiance inspection commands, and the entity payload retains the latest eight changes, including the responsible player for trusted reassignment. Village naming Use any banner on a bell inside an active tracked village. The banner stays in the player's hand. A non operator must have Revered or Royalty reputation with at least half (rounded up) of the tracked living adult residents. Names are 1–32 characters, whitespace normalized, formatting free, and unique across tracked identities. Debug visualization Set debugOverlay.showVillageBounds in the generated config screen. The default is false. One option controls the complete feature: enabling subscribes to a server preview of active and empty grace villages within 256 blocks; world outlines use the actual POI section union with internal coplanar edges removed; a gold center marker matches the existing job site debug treatment; the canonical village name appears as plain shadowed gold text at the top center only while the player is inside its footprint; F1 hides the HUD label with the rest of the GUI; disabling, logout, dimension changes, server unsubscribe, and preview timeout clear cached geometry and labels. The preview never force loads chunks, sends archived footprints, or updates players who are not subscribed. Payloads are capped at 64 villages, 512 sections per village, and 4,096 sections total. Administration",
      "headings": [
        {
          "level": 3,
  ...
  "examples": [
    ...
    },
    {
      ...
      "level": "Advanced",
      "language": "jsonc",
      "code": "\"professions\": [\"minecraft:farmer\", \"minecraft:fletcher\"]"
    },
    {
    ...
    },
    {
      ...
      "page": "pacification",
      "pageTitle": "Pacification",
      "section": "Example: Modded Currency",
      "level": "Advanced",
      "language": "json",
      "code": "{\n  \"payments\": [\n    {\n      \"id\": \"my_pack.pacification.coins\",\n      \"item\": \"numismatic-overhaul:gold_coin\",\n      \"count\": 12,\n      \"name\": \"gold coin\",\n      \"plural_name\": \"gold coins\",\n      \"priority\": 20\n    }\n  ]\n}"
    },
    {
      ...
      "page": "pacification",
      "pageTitle": "Pacification",
      "section": "Example: Profession-Specific Cost",
      "level": "Advanced",
      "language": "json",
      "code": "{\n  \"payments\": [\n    {\n      \"id\": \"my_pack.pacification.toolsmith\",\n      \"professions\": [\"minecraft:toolsmith\"],\n      \"items\": [\"minecraft:iron_ingot\"],\n      \"min_count\": 2,\n      \"max_count\": 4\n    }\n  ]\n}"
    },
    ...
    },
    {
      ...
      "page": "sell-box-and-daily-market",
      "pageTitle": "Sell Box and Daily Market",
      "section": "Override, add, or disable",
      "level": "Advanced",
      "language": "json",
      "code": "{\n  \"enabled\": false\n}"
    },
    ...
    },
    {
      ...
      "level": "Advanced",
      "language": "json",
      "code": "\"professions\": [\"examplemod:guard\", \"examplemod:archer\"]"
    },
    {
